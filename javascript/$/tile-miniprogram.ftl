好的，已将原注释中所有描述布局结构的 ASCII 艺术图完整还原。以下是微信小程序版本的完整 FTL 模板文件：
...
${""?left_pad(indent)}    <text class="tile-status">{{ row.${guidbase.name_widget_variable(guidbase.get_child_from_tile(widget, "status"))} }}</text>
  ...
  <#if guidbase.has_child_widget(widget, "secondary")>
${""?left_pad(indent)}    <view class="tile-avatar">
${""?left_pad(indent)}      <image src="{{ row.${guidbase.name_widget_variable(guidbase.get_child_from_tile(widget, "secondary"))} }}"></image>
${""?left_pad(indent)}    </view>
  </#if>
${""?left_pad(indent)}  </view>
${""?left_pad(indent)}  <view class="tile-row tile-footer">
