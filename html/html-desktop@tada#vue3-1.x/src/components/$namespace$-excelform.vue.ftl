...
/*
 * ═══════════════════════════════════════════════════════
 *  Excel Grid — Microsoft Excel 风格
 *  Grid lines | Header bg | Cell selection | Resize
 * ═══════════════════════════════════════════════════════
 */
...
.xls-wrap {
  /* Grid */
  --xls-grid-line:    #c5c5c5;
  --xls-grid-pale:    #e0e0e0;

  /* Headers */
  --xls-hdr-bg:       #f3f3f3;
  --xls-hdr-bg-hover: #e0e0e0;
  --xls-hdr-text:     #444;
  --xls-hdr-border:   #c0c0c0;

  /* Data cells */
  --xls-cell-bg:      #ffffff;
  --xls-cell-text:    #222;

  /* Selection (classic Excel blue) */
  --xls-sel-outline:  #217346;
  --xls-sel-bg:       rgba(33, 115, 70, .10);
  --xls-sel-range-bg: rgba(33, 115, 70, .06);

  /* Editor */
  --xls-edit-outline: #217346;
  --xls-edit-shadow:  0 0 0 2px rgba(33, 115, 70, .25);

  /* Misc */
  --xls-row-num-w:     48px;
  --xls-cell-h:        26px;
  --xls-font:          12px;
  --xls-font-family:   'Segoe UI', -apple-system, BlinkMacSystemFont, system-ui, sans-serif;
}
...
.xls-wrap {
  ...
  overflow: hidden;
  font-family: var(--xls-font-family);
  font-size: var(--xls-font);
  color: var(--xls-cell-text);
}
...
.xls-sheet::-webkit-scrollbar { width: 11px; height: 11px; }
.xls-sheet::-webkit-scrollbar-track { background: #f1f1f1; }
.xls-sheet::-webkit-scrollbar-thumb {
  background: #c1c1c1;
  border-radius: 6px;
  border: 2px solid #f1f1f1;
}
.xls-sheet::-webkit-scrollbar-thumb:hover { background: #a0a0a0; }
.xls-sheet::-webkit-scrollbar-corner { background: #f1f1f1; }
...
.xls-corner {
  ...
  background: var(--xls-hdr-bg);
  border-right:  1px solid var(--xls-hdr-border);
  border-bottom: 1px solid var(--xls-hdr-border);
  width:  var(--xls-row-num-w);
  min-width: var(--xls-row-num-w);
  height: var(--xls-cell-h);
}
...
th.xls-ch {
  ...
  background: var(--xls-hdr-bg);
  border-right:  1px solid var(--xls-grid-pale);
  border-bottom: 1px solid var(--xls-hdr-border);
  text-align: center;
  font-weight: 600;
  color: var(--xls-hdr-text);
  padding: 0 6px;
  height: var(--xls-cell-h);
  cursor: pointer;
  user-select: none;
...
th.xls-ch:hover { background: var(--xls-hdr-bg-hover); }
th.xls-ch.sc {
  background: #e2e8e4;
  color: #1a5c38;
}
...
th.xls-rh {
  ...
  background: var(--xls-hdr-bg);
  border-right:  1px solid var(--xls-hdr-border);
  border-bottom: 1px solid var(--xls-grid-pale);
  text-align: center;
  font-weight: 400;
  color: var(--xls-hdr-text);
  padding: 0 2px;
...
th.xls-rh:hover { background: var(--xls-hdr-bg-hover); }
th.xls-rh.sr {
  background: #e2e8e4;
  color: #1a5c38;
  font-weight: 600;
...
/* ── Data cells ──────────────────────────────── */
td {
  border-right:  1px solid var(--xls-grid-line);
  border-bottom: 1px solid var(--xls-grid-line);
  background: var(--xls-cell-bg);
  padding: 0;
  height: var(--xls-cell-h);
...
/* Selected cell(s) */
td.sel {
  background: var(--xls-sel-bg) !important;
  outline: 2px solid var(--xls-sel-outline);
  outline-offset: -1px;
  z-index: 3;
  overflow: visible;
}
td.inr {
  background: var(--xls-sel-range-bg) !important;
}
...
.xls-cbc input {
  ...
  accent-color: var(--xls-sel-outline);
  cursor: pointer;
}

/* Dropdown / Date hint icon in cell */
.xls-drop-hint {
  font-size: 10px;
  color: #999;
  flex-shrink: 0;
...
.xls-input {
  ...
  padding: 0 5px;
  border: none;
  outline: 2px solid var(--xls-edit-outline);
  font-size: var(--xls-font);
  font-family: inherit;
  background: #fff;
  box-shadow: var(--xls-edit-shadow);
}

/* ── Dropdown trigger (open state) ───────────── */
.xls-drop-trigger {
...
.xls-resize:hover { background: rgba(33, 115, 70, .3); }
...
.xls-drop-item.active { background: #e2e8e4; font-weight: 600; color: #1a5c38; }
...
.xls-date-head {
  ...
  color: #fff;
  display: flex; align-items: center;
  height: 30px; padding: 0 4px; gap: 2px;
}
.xls-date-nav {
...
.xls-date-wd:first-child,
.xls-date-wd:last-child { color: #d32f2f; }
...
.xls-date-day:hover { background: #e8f0fe; }
.xls-date-day--other { color: #bbb; }
.xls-date-day--today {
  font-weight: 700;
  color: #217346;
...
.xls-date-day--weekend { color: #d32f2f; }
...
.xls-date-my-item:hover { background: #e8f0fe; }
.xls-date-my-item--selected {
  background: #217346; color: #fff; font-weight: 700;
}
...
.xls-date-today {
  font-size: 11px; color: #217346; cursor: pointer;
  padding: 2px 5px; border-radius: 3px;
}
.xls-date-today:hover { background: #e8f0fe; }
.xls-date-clear {
  font-size: 11px; color: #999; cursor: pointer;
  padding: 2px 5px; border-radius: 3px;
}
.xls-date-clear:hover { color: #d32f2f; background: #fce4ec; }
...
.xls-cxi:hover { background: #e8f0fe; }
.xls-cxi i {
  font-size: 14px;
  color: #666;
  width: 18px;
