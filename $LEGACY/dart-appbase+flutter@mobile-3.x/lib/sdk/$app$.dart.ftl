...
<#list pages![] as page>
  ...
  static String convertDistrict2Text(Map district) {
    if (district == null) return "";
    String ret = "";
    if (district["provinceCode"] != null && district["provinceName"] != null) {
      ret += district["provinceName"];
    }
    if (district["cityCode"] != null && district["cityName"] != null && district["cityName"] != "市辖区" && district["cityName"] != "县") {
      ret += district["cityName"];
    }
    if (district["countyCode"] != null && district["countyName"] != null) {
      ret += district["countyName"];
    }
    if (district["townCode"] != null && district["townName"] != null) {
      ret += district["townName"];
    }
    return ret;
  }

  static String convertValue2Text(String name, dynamic value) {
    if (value == null) return "";
    if (${app.name}.options[name] == null) return value.toString();
    Map opt = ${app.name}.options[name];
    if (opt["values"] == null) return value.toString();
    if (value.runtimeType.toString() == "List<String>") {
      ...
      for (int i = 0; i < opt["values"].length; i++) {
        Map row = opt["values"][i];
        if (row["value"] == value) {
          return row["text"];
        }
      }
  ...
  static bool isOptionInValue(String name, int index, Map values) {
    if (values[name] == null) return false;
    if (${app.name}.options[name] == null) return false;
    Map opt = ${app.name}.options[name];
    if (opt["values"] == null) return false;
    return values[name].contains(opt["values"][index]["value"]);
  }

  static void addOptionToValue(String name, int index, Map values) {
    if (values[name] == null) return;
    if (${app.name}.options[name] == null) return;
    Map opt = ${app.name}.options[name];
    if (opt["values"] == null) return;
    values[name].add(opt["values"][index]["value"]);
  }

  static void removeOptionFromValue(String name, int index, Map values) {
    if (values[name] == null) return;
    if (${app.name}.options[name] == null) return;
    Map opt = ${app.name}.options[name];
    if (opt["values"] == null) return;
    values[name].remove(opt["values"][index]["value"]);
  }

  static void toggleOptionInValue(String name, int index, Map values) {
    if (values[name].runtimeType.toString() == "List<String>") {
      if (isOptionInValue(name, index, values)) {
        removeOptionFromValue(name, index, values);
      } else {
        addOptionToValue(name, index, values);
      }
    } else {
      if (values[name] == ${app.name}.options[name]["values"][index]["value"]) {
        values[name] = "";
      } else {
        values[name] = ${app.name}.options[name]["values"][index]["value"];
      }
    }
  }

  static bool isOptionNotGreaterThanValue(String name, int index, Map values) {
    if (values[name] == null) return false;
    if (${app.name}.options[name] == null) return false;
    Map opt = ${app.name}.options[name];
    if (opt["values"] == null) return false;
    return opt["values"][index]["value"].compareTo(values[name]) <= 0;
  }

  static bool isOptionEqualsToValue(String name, int index, Map values) {
    if (values[name] == null) return false;
    if (${app.name}.options[name] == null) return false;
    Map opt = ${app.name}.options[name];
    if (opt["values"] == null) return false;
...
<#list methods as method>
   ...
   */
  static Future<dynamic> ${method}(params) async {
    <#if !widget.tile??>
      <#assign widget = widget + {'tile': {}}>
    </#if>
    <#if widget.widgetType == '滚动导航'>
    return [{
   ...
   */
  static Future<Map> ${method}(${js.nameVariable(widget.variable)}) async {
    return {};
  }
   ...
   */
  static Future<Map> ${method}(id) async {
  <#assign custom = {}>
